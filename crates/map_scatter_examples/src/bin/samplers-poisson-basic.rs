...
fn main() -> anyhow::Result<()> {
    ...
    // Use Poisson-disk sampling to ensure minimum spacing between points
    let plan = Plan::new().with_layer(Layer::new(
        "poisson_layer",
        vec![dots],
        Box::new(PoissonDiskSampling::new(3.0)), // minimum radius between points
    ));
    ...
    // Render to PNG
    let mut rc = RenderConfig::new(image_size, domain_extent).with_background([30, 30, 30]);
    rc.set_kind_style(
        "dots",
        KindStyle::Circle {
            color: [240, 240, 240],
    ...
    let out = "samplers-poisson-basic.png";
